...
namespace TheBlueAllianceOffline
{
    public class Events
    {
        ...
        public static TheBlueAlliance.Models.Events.Event[] GetEvents(int year)
        {
            var dataList = new List<TheBlueAlliance.Models.Events.Event>();
            var wc = new WebClient();
            wc.Headers.Add("X-TBA-App-Id", "3710-xNovax:FRC_Scouting_V2:" + Assembly.GetExecutingAssembly().GetName().Version);
            try
            {
                string url = ("http://www.thebluealliance.com/api/v2/events/" + year);
                dataList = JsonConvert.DeserializeObject<List<TheBlueAlliance.Models.Events.Event>>(wc.DownloadString(url));
            }
            catch (Exception webError)
            {
                Console.WriteLine("Error Message: " + webError.Message);
